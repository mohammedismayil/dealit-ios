//
//  WalletConnectVC.swift
//  Dealit
//
//  Created by Mohammed Ismayil on 12/04/22.
//
...
//    var scannerController: ScannerViewController?
    ...
    let privateKey: EthereumPrivateKey = try! EthereumPrivateKey(
        privateKey: .init(hex: "31f31a1674e45b7c5df4f1adfc7261c8a09ad3bc3e6dec6a23ac2a916e20e4e6"))
...
//    https://example.walletconnect.org/ --use this url and get wcurl change below and check the every method called from the website(mentioned here) to update here-- for example eth_sendTransaction need approval from the app
        ...
        guard let url = WCURL("wc:441368c0-4601-41b8-80ba-237abf43d615@1?bridge=https%3A%2F%2Fs.bridge.walletconnect.org&key=49d096d430caf8fc0b0a1081f71c7e318b58d6b7e8e45f492ff0e8fc4cc5eef1") else { return }
...
extension WalletConnectVC: ServerDelegate {
    ...
    func server(_ server: Server, shouldStart session: Session, completion: @escaping (Session.WalletInfo) -> Void) {
        ...
        onMainThread {
            UIAlertController.showShouldStart(from: self, clientName: session.dAppInfo.peerMeta.name, onStart: {
                ...
                completion(Session.WalletInfo(approved: false, accounts: [], chainId: 3, peerId: "", peerMeta: walletMeta))
                self.scanQRCodeButton.isEnabled = true
            })
        }
    }
